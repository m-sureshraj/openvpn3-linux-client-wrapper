...
function start(options = []) {
    const { VPN_PROFILE_PATH } = process.env;
    if (!VPN_PROFILE_PATH) {
        process.stderr.write(red('Environment variable `VPN_PROFILE_PATH` not defined. Refer to the setup guide.\n'));
        process.exit();
    }

    if (options.length === 0) {
        startSessionManual(VPN_PROFILE_PATH);
    } else {
        ...
        startSessionAuto(username, password, VPN_PROFILE_PATH);
    }
}
